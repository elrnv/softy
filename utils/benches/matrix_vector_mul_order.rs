...
pub fn lazy_expr4(m: ChunkedN<&[f64]>, v: &[f64]) -> Vec<f64> {
    let n = v.len() / 4;
    let m4 = ChunkedN::from_flat_with_stride(Chunked4::from_flat(Chunked4::from_flat(m.into_flat())), n);
...
pub fn inner_outer_simd(m: ChunkedN<&[f64]>, v: &[f64]) -> Vec<f64> {
    ...
    for (rows4, outs4) in m.chunks_exact(4*n).zip(out.chunks_exact_mut(4)) {
        unsafe {
            ...
            for (&r0, &r1, &r2, &r3, &col) in zip!(row0.iter(), row1.iter(), row2.iter(), row3.iter(), v.iter()) {
...
pub fn inner_par_chunked(m: ChunkedN<&[f64]>, v: &[f64]) -> Vec<f64> {
    ...
    let chunk_size = size / n;
    let m = m.into_flat();
    m.par_chunks(size*chunk_size).zip(out.par_chunks_mut(chunk_size)).for_each(|(m, out)| {
        m.chunks(size).zip(out.iter_mut()).for_each(|(row, out_val)| {
            row.iter().zip(v.iter()).for_each(|(&col, &rhs)|  {
                *out_val += col*rhs;
            });
        });
...
fn matrix_vector_mul_order_benchmark(c: &mut Criterion) {
    ...
    for &n in &[100, 200, 400, 800, 1600, 3200/*, 5000, 7500, 10000*/] {
        ...
        assert_eq!(inner(m.view(), v.view()), outer_read_local(m.view(), v.view()));
        assert_eq!(inner_par(m.view(), v.view()), outer_read_local(m.view(), v.view()));
        ...
        approx_equal(outer_read_local_par(m.view(), v.view()), outer_write_local(m.view(), v.view()));
        ...
        group.bench_with_input(BenchmarkId::new("Lazy Expr Blocks", n), &(&m, &v),
            |b, (m, v)| {
                b.iter_batched(|| (m.view(), v.view()), |(m,v)| lazy_expr4(m, v), BatchSize::LargeInput)
            });
        ...
        group.bench_with_input(BenchmarkId::new("Inner Parallel Chunked", n), &(&m, &v),
            |b, (m, v)| {
                b.iter_batched(|| (m.view(), v.view()), |(m,v)| inner_par_chunked(m, v), BatchSize::LargeInput)
            });
        #[cfg(feature="packed_simd")]
        {
            approx_equal(inner_par(m.view(), v.view()), inner_simd(m.view(), v.view()));
            approx_equal(inner_par(m.view(), v.view()), inner_outer_simd(m.view(), v.view()));
            group.bench_with_input(BenchmarkId::new("Inner SIMD", n), &(&m, &v),
                |b, (m, v)| {
                    b.iter_batched(|| (m.view(), v.view()), |(m,v)| inner_simd(m, v), BatchSize::LargeInput)
                });
            group.bench_with_input(BenchmarkId::new("Inner Outer SIMD", n), &(&m, &v),
                |b, (m, v)| {
                    b.iter_batched(|| (m.view(), v.view()), |(m,v)| inner_outer_simd(m, v), BatchSize::LargeInput)
                });
        }
    }
