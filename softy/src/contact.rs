...
mod tests {
    ...
    fn contact_physical_space_conversion_test() -> Result<(), crate::Error> {
        ...
        let run = |trimesh: TriMesh<f64>| -> Result<(), crate::Error> {
            ...
            geo::algo::compute_vertex_area_weighted_normals(trimesh.vertex_positions(),
                reinterpret::reinterpret_slice(trimesh.indices.as_slice()), &mut normals);
            ...
            let contact_vecs = friction.to_contact_space(reinterpret::reinterpret_vec(vecs.clone()));
