...
pub struct ImplicitContactConstraint {
    ...
    pub contact_points: RefCell<Vec<[f64; 3]>>,
...
impl ImplicitContactConstraint {
    ...
    pub fn new(
        // The solid being affected by the contact.
        solid: &TetMeshSolid,
        // The collision object that generates an implicit surface.
        shell: &TriMeshShell,
        kernel: KernelType,
        ...
        if let Some(surface) = surface_builder.build() {
            ...
            let contact_points = surf_mesh.vertex_positions();
...
impl ContactConstraint for ImplicitContactConstraint {
            ...
            let velocity_t: Vec<_> = surf_indices
                ...
                .map(|(contact_idx, &surf_idx)| {
                    ...
                    let v = frictional_contact
                        .contact_basis
                        .to_cylindrical_contact_coordinates(v[vtx_idx], contact_idx);
                    v.tangent
                })
                .collect();
    ...
    fn update_cache(&mut self, sim_pos: &[f64;3], col_pos: &[f64; 3]) -> bool {
        ...
        surf.cache_neighbours(&self.contact_points.borrow())
    }
...
impl Constraint<f64> for ImplicitContactConstraint {
    ...
    fn constraint(&self, _x0: &[f64], x1: &[f64], value: &mut [f64]) {
        ...
        surf.potential(&contact_points, &mut cbuf).unwrap();
...
impl ConstraintJacobian<f64> for ImplicitContactConstraint {
    ...
    fn constraint_jacobian_values(
        ...
        Ok(self
            .implicit_surface
            .borrow()
            .query_jacobian_values(&contact_points, values)?)
    }
}
...
impl ConstraintHessian<f64> for ImplicitContactConstraint {
    ...
    fn constraint_hessian_values(
        ...
        Ok(self
            .implicit_surface
            .borrow()
            .query_hessian_product_scaled_values(&contact_points, lambda, scale, values)?)
    }
}
