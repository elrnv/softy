...
fn box_bench(c: &mut Criterion) {
    ...
    for i in (2..38).step_by(2) {
        ...
        // 100x100 grids
        let (top_grid, bottom_grid) = make_grids(100);

        group.bench_function(BenchmarkId::new("Box Point Contact Grid 100", i), |b| {
            let mut engine = SolverBuilder::new(DYNAMIC_PARAMS)
                .add_solid(contact_box.clone(), stretch_material())
                .add_fixed(top_grid.clone(), 1)
                .add_fixed(bottom_grid.clone(), 1)
        ...
        group.bench_function(BenchmarkId::new("Box Linearized Point Contact Grid 100", i), |b| {
            let mut engine = SolverBuilder::new(DYNAMIC_PARAMS)
                .add_solid(contact_box.clone(), stretch_material())
                .add_fixed(top_grid.clone(), 1)
                .add_fixed(bottom_grid.clone(), 1)
