...
pub trait ConstraintJacobian<T: Scalar> {
    ...
    fn constraint_jacobian(&mut self) -> &[Vector3<T>];
}
...
/// functions.
pub trait ConstraintHessianIndicesValues<T: Scalar>: EnergyHessianSize {
    ...
    fn constraint_hessian_values(&mut self, lambda: &[T]) -> &[T];
}
...
/// constraint function had dimension 1 and `lambda` was given to be 1, then this trait provides
/// the true Hessian of the constraint function.
pub trait ConstraintHessian<T: Scalar>: EnergyHessianSize {
    ...
    fn constraint_hessian(&mut self, lambda: &[T]) -> &[MatrixElementTriplet<T>];
}
