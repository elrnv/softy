...
pub(crate) mod test_utils {
              ...
              E: Energy<F> + EnergyGradient<F>
    {
        for mesh in test_meshes().into_iter() {
            ...
            for i in 0..x.len() {
                ...
                assert_relative_eq!(grad[i].value(), energy.deriv(), max_relative = 1e-6, epsilon = 1e-10);
                dx[i] = F::cst(dx[i]);
            }
        }
    }
    ...
    {
        use crate::matrix::{MatrixElementTriplet as Triplet, MatrixElementIndex as Index};
        ...
        for mesh in test_meshes().into_iter() {
            ...
            let mut hess_triplets = vec![Triplet::new(0,0,F::zero()); energy_model.energy_hessian_size()];
            energy_model.energy_hessian(&x, &dx, &mut hess_triplets);

            // Build a dense hessian
            let mut hess = vec![vec![F::zero(); x.len()]; x.len()];
            for Triplet { idx: Index { row, col }, val } in hess_triplets.into_iter() {
                hess[row][col] += val;
                if row != col {
                    hess[col][row] += val;
                }
            ...
            for i in 0..x.len() {
                ...
                energy_model.add_energy_gradient(&x, &dx, &mut grad);
                for j in 0..x.len() {
                    assert_relative_eq!(hess[i][j].value(), grad[j].deriv(), max_relative = 1e-6, epsilon = 1e-10);
                }
                dx[i] = F::cst(dx[i]);
            }
        }
